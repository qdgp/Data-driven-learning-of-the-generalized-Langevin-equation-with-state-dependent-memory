function step5_hx_GLE_1D(T_scale,N_per_step,M,save_file_name,random_seed)
theta_cut=40;
load('data/PDF.mat','ff_x','ff_f','kT','v_square','dt','hx_x','hx');
load('data/hx_corr.mat','corr_invh_maf_v','corr_hv_v');
mass = kT/v_square;
hx_x=[0;hx_x;40];
hx_hx=[hx(1);hx;hx(end)];
fx=@(x) kT*interp1(ff_x,ff_f, x, 'linear','extrap');
hx=@(x) interp1(hx_x,hx_hx, x, 'linear','extrap');   % (1,3)

T_cut=200;T=T_cut*T_scale;
dw=2*pi/T/2;

N_sample = T_cut/dt; 
g = corr_invh_maf_v; h = corr_hv_v;
...
hx_x=hx(x(1,:)); % hx(x(n,:)) (1,M)  fx(x(n,:)) (1,M)
...
tic
for n=2:N-1
    hx_x=hx(x(n,:)); % hx(x(n,:)) (1,M)  fx(x(n,:)) (1,M)
    F_history=hx_x.*(theta'*vh(n+Ntheta-1:-1:n,:))*dt; %(1,M)
    Fv=fx(x(n,:))-F_history-1/2*theta0*hx_x.*v(n,:).*hx_x*dt+hx_x.*R(n,:);   %(1,M)
