...
class get_model(object):
    def __init__(self,T_cut,N_hx,hx_x,ND,a):
        ...
        self.N_Theta=int(self.N/2)
        self.N_pad=self.N-self.N_Theta
        self.N_hx=N_hx
        self.a=a
        self.weights={}
        self.weights['Theta_sym'] =tf.Variable(0.1*random_normal([self.N_Theta,ND,ND],dtype=FT),trainable=True,name='Theta_sym')
        self.weights['hx']=tf.Variable(-1.4*random_normal([ND,self.N_hx]),trainable=True,name='hx')
    ...
    def get_Theta(self):
        ...
        hx=(20.0*tf.math.sigmoid(self.weights['hx']))/np.sqrt(float(self.ND))
